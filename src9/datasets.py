...
from src9.utils import load, padTensor, get_max_len
...
def get_dataset_iemocap(data_folder: str, phase: str, img_interval: int, hand_crafted_features: Optional[bool] = False):
    ...
    meta = load(os.path.join(main_folder, 'meta1.pkl'))

    emoDict = getEmotionDict()
    uttr_ids = open(os.path.join(data_folder, 'IEMOCAP_SPLIT', f'{phase}_split1.txt'), 'r').read().splitlines()
    texts = [meta[uttr_id]['text'] for uttr_id in uttr_ids]
    labels = [emoDict[meta[uttr_id]['label']] for uttr_id in uttr_ids]
    senti = [meta[uttr_id]['senti'] for uttr_id in uttr_ids]
...
def get_dataset_mosei(data_folder: str, phase: str, img_interval: int, hand_crafted_features: Optional[bool] = False):
    ...
    texts = [meta[idd]['text'] for idd in ids]
    labels = [meta[idd]['label'] for idd in ids]
    senti = [meta[idd]['senti'] for idd in ids]
    if hand_crafted_features:
...
class MOSEI(Dataset):
    def __init__(self, main_folder: str, ids: List[str], texts: List[List[int]], labels: List[int], img_interval: int, senti):
    ...
    def sample_imgs_by_interval(self, folder: str, fps: Optional[int] = 30) -> List[str]:
        ...
        v_feature = [os.path.join(folder+"_vfeature", f'image_{i}.pkl') for i in list(range(0, nums, step))]
        if len(sampled) == 0:
            step = int(self.img_interval / 1000 * fps) // 4
            sampled = [os.path.join(folder, f'image_{i}.jpg') for i in list(range(0, nums, step))]
            v_feature = [os.path.join(folder+"_vfeature", f'image_{i}.pkl') for i in list(range(0, nums, step))]        
        return sampled,v_feature
    ...
    def __getitem__(self, ind: int) -> Tuple[str, np.array, List[torch.tensor], List[int], np.array]:
        ...
        zeros=[0.0 for i in range(711)]
        sampledImgs = []
        imgPaths,vfeature_path=self.sample_imgs_by_interval(sample_folder)
        ...
        list_vfeature=[]
        len1=len(vfeature_path)
        bu=[]
        bu1=[]
        for i in range(len1):
            if os.path.exists(vfeature_path[i]):
                list_vfeature.append(pickle.load(open(vfeature_path[i],'rb')))
                bu1.append(i)
            else:
                list_vfeature.append(zeros)
                bu.append(i)
        #print(len1)
        len2=len(bu)
        #print(len2)
        if len2!=0 and len2<len1:
            for i in range(len2):
                list_vfeature[bu[i]]=list_vfeature[bu1[0]]        


        waveform, sr = torchaudio.load(os.path.join(sample_folder, f'audio.wav'))

        afeature_path = os.path.join(sample_folder+"_vfeature", f'audio.pkl')
        list_afeature = pickle.load(open(afeature_path,'rb'))
        ...
        if len(list_afeature)<len(specgrams):
            tmp=list_afeature[-1]
            list_afeature.append(tmp)
        if len(list_afeature)!=len(specgrams):
            print(len(list_afeature))
            print(len(specgrams))
            print(sr)
            print(waveform.size())
            ...
            print("音频特征维度不对应")
            exit(0)
        video_feature = torch.tensor(list_vfeature,dtype=torch.float32)
        audio_feature = torch.tensor(list_afeature,dtype=torch.float32)
        return this_id, sampledImgs, specgrams, self.texts[ind], self.labels[ind],video_feature, audio_feature, self.senti[ind]

class IEMOCAP(Dataset):
    def __init__(self, main_folder: str, utterance_ids: List[str], texts: List[List[int]], labels: List[int],
                 label_annotations: List[str], img_interval: int, senti):
    ...
    def sample_imgs_by_interval1(self, folder: str, imgNamePrefix: str, fps: Optional[int] = 30) -> List[str]:#原函数，备份
        '''
        Arguments:
    ...
    def sample_imgs_by_interval(self, folder: str, imgNamePrefix: str, fps: Optional[int] = 30) -> List[str]:
        ...
        for i in list(range(0, nums, step)):
            sampled.append(os.path.join(folder, f'{imgNamePrefix}{i}.jpg'))
            vfeature.append(os.path.join(folder+"_vfeature", f'{imgNamePrefix}{i}.pkl'))
        #sampled = [os.path.join(folder, f'{imgNamePrefix}{i}.jpg') for i in list(range(0, nums, step))]
        # print(sampled)
        # print(vfeature)
        # exit(0)
        return sampled,vfeature
    ...
    #     # exit(0)
    #     #print(os.path.join(uttrFolder, f'audio_{audio_suffix}.wav'))
    ...
    def __getitem__(self, ind: int) -> Tuple[str, np.array, List[torch.tensor], List[int], np.array]:
        ...
        imgs_path, vfeature_path=self.sample_imgs_by_interval(uttrFolder, imgNamePrefix)
        list1=[]
        for imgPath in imgs_path:
            list1.append(np.float32(Image.open(imgPath)))
        sampledImgs = np.array(list1)
        #print(sampledImgs.shape)
        # exit(0)
        #print(os.path.join(uttrFolder, f'audio_{audio_suffix}.wav'))
        zeros=[0.0 for i in range(711)]
        ...
        afeature_path = os.path.join(uttrFolder+"_vfeature", f'audio_{audio_suffix}.pkl')
        list_afeature = pickle.load(open(afeature_path,'rb'))
        #print(waveform.size())
        # print(sr)
        # exit(0)
        ...
        video_feature = torch.tensor(list_vfeature,dtype=torch.float32)
        audio_feature = torch.tensor(list_afeature,dtype=torch.float32)
        return uttrId, sampledImgs, specgrams, self.texts[ind], self.labels[ind], video_feature, audio_feature, self.senti[ind] 
...
def collate_fn(batch):
    ...
    sentis=[]
    for dp in batch:
        utteranceId, sampledImgs, specgram, text, label, video_feature, audio_feature, senti = dp
    ...
    return (
        ...
        torch.tensor(sentis, dtype=torch.float32)
    )
